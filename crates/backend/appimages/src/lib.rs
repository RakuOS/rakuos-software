// rakuos-appimages — AppImage management
...
fn install_dir() -> PathBuf { home().join(".local/share/rakuos/appimages") }
fn icon_dir()    -> PathBuf { home().join(".local/share/rakuos/appimages/icons") }
fn desktop_dir() -> PathBuf { home().join(".local/share/applications") }
fn extract_tmp() -> PathBuf {
    std::env::temp_dir().join("rakuos-appimage-extract")
}

const DESKTOP_PREFIX: &str = "rakuos-appimage-";
